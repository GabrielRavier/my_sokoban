...
static signed char get_digit(
    const struct my_bigint *smaller_greater_num[2], size_t i, bool carry)
{
...
static size_t do_sub_loop(
    struct my_bigint *result, const struct my_bigint *smaller_greater_num[2])
{
    size_t end_size = 1;
    signed char digit;
    bool carry = false;
