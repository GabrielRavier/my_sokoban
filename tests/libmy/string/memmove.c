...
Test(my_memmove, osv_tests)
{
    ...
    for (size_t i = 0; i < 8; ++i) {
        ...
        my_memmove(&buf_tmp[8], &buf_tmp[i], 8);
    ...
    for (size_t i = 0; i < 8; ++i) {
        ...
        my_memmove(&buf_tmp[i], &buf_tmp[8], 8);
        cr_assert_eq(my_strncmp(loop_results[i], buf_tmp, 16), 0);
    }
