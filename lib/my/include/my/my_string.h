...
static inline struct my_string *my_string_insert_char(
    struct my_string *self, char character, size_t position)
{
...
static inline struct my_string *my_string_append_char(
    struct my_string *self, char character)
{
    return my_string_append(self, &character, 1);
}

// Removes count characters starting at index
struct my_string *my_string_erase(
    struct my_string *self, size_t index, size_t count);
