    ...
    va_list *arguments, struct my_printf_conversion_info *format_info)
{
    if (format_info->length_modifier == PRINTF_LENGTH_MODIFIER_INTMAX) {
        *va_arg(*arguments, intmax_t *) = (intmax_t)destination->length;
        return NULL;
    }
    if (format_info->length_modifier == PRINTF_LENGTH_MODIFIER_SIZE_T) {
        *va_arg(*arguments, ssize_t *) = (ssize_t)destination->length;
        return NULL;
    }
    if (format_info->length_modifier == PRINTF_LENGTH_MODIFIER_PTRDIFF_T) {
        *va_arg(*arguments, ptrdiff_t *) = (ptrdiff_t)destination->length;
        return NULL;
    }
    *va_arg(*arguments, int *) = (int)destination->length;
    return NULL;
}
    ...
    va_list *arguments, struct my_printf_conversion_info *format_info)
{
    if (format_info->length_modifier == PRINTF_LENGTH_MODIFIER_CHAR) {
        *va_arg(*arguments, signed char *) = (signed char)destination->length;
        return NULL;
    }
    if (format_info->length_modifier == PRINTF_LENGTH_MODIFIER_SHORT) {
        *va_arg(*arguments, short *) = (short)destination->length;
        return NULL;
    }
    ...
    if (format_info->length_modifier == PRINTF_LENGTH_MODIFIER_LONG_LONG) {
        *va_arg(*arguments, long long *) = (long long)destination->length;
        return NULL;
    }
